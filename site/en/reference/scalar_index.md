...
As shown in the figure above, Milvus abstracts the attribute filtering condition (`expr`) into an appropriate function. In each segment, Milvus uses this function to generate a bitset and applies the bitset to the similarity search along with the query vector. 
