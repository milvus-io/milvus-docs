...
If the amount of data is less than the upper limit of a single insertion (256 MB), batch insertion is much more efficient than single insertion.

Two parameters in the system configurations have an impact on the insertion performance:
...
This parameter (by default 1 second) refers to the interval time of the data flushing task in the background. According to the [data merging strategy](storage_operation.md#Data Merge) of Milvus, increasing this value can reduce the number of segment merges, reduce disk I/O, and increase the throughput rate of insert operations.

<div class="alert note">
Milvus cannot search for data that has not been flushed within this time interval.
</div>

Besides, the parameter `index_file_size`, which is used when creating collections, has an impact on the insertion performance. The value of this parameter is 1024 MB by default and 4096 MB at most. The larger the `index_file_size`, the more times it takes to merge data to the size set by this parameter, which affects the throughput rate of the insert operation. The smaller the parameter, the more data segments are generated, which may deteriorate the query performance.

In addition to software-level factors, network bandwidth and storage media also influence insertion performance.
...
This parameter (by default 4 GB) refers to the size of the cache space used for resident query data. If the cache space is insufficient to hold the required data, the data will be temporarily loaded from the disk during the query, which seriously affects query performance. Therefore, `cpu_cache_capacity` should be greater than the amount of data required by the query.
...
<div class="alert note">
By calling the <code>get_collection_stats</code> API, you can get the total amount of data required to query a collection.
</div>


- `gpu_config.gpu_search_threshold`

In the GPU version, GPU is enabled for query when the number of target vectors is greater than or equals to the `gpu_search_threshold` (by default 1000).

The performance of GPU queries depends on the speed at which the CPU loads data into graphic memory and the computing power of the GPU. The advantages of parallel computing with GPUs cannot be fully utilized when processing a small number of target vectors. Only when the number of target vectors reaches a certain threshold, the query performance on GPUs will be better than on CPUs. In practice, the ideal value of this parameter can be obtained based on experimental comparison.

- `gpu_resource_config`

Specifies the GPU devices used for querying or indexing. For scenarios where data insertion and querying are concurrent, you can use GPUs to build indexes so as to avoid the index building task competing for CPU resources with the query task. For scenarios with a large number of query target vectors, using multiple GPUs can significantly improve query efficiency.
...
To choose the right index, you need to weigh multiple indicators such as storage space, query performance, and query recall rate.
...
<div class="alert note">
In the scenario with continuous data insertion, since the system does not index the segments with a size less than <code>index_file_size</code>, it uses brute-force search as the query method. The amount of calculation can be estimated by multiplying the number of target vectors by the total number of segment vectors.
</div>


- HNSW / RNSG / ANNOY index

The index parameters of HNSW, RNSG, and ANNOY have a more complex impact on query performance. It is recommended to see [Index Introduction](index_type.md#index introduction) for details.
...
When querying data for the first time, the system needs to read the data from the disk and write the data to the cache, which is a time-consuming process. To avoid loading data during the first query, you can call the `load_collection` API in advance, or use the system parameter `preload_collection` to specify the collection to be preloaded when starting Milvus.

- Compact segments

As mentioned in [compact segments](storage_operation.md#Compact segments), in order to filter deleted entities, Milvus reads **delete_docs** into memory when querying data. You call the `compact` API to clean up deleted entities and reduce filtering operations, thereby improving query performance.

## Optimize storage

- Compact segments

As mentioned in [compact segments](storage_operation.md#Compact segments), deleted entities do not participate in the calculation and takes up disk space. If a large number of entities have been deleted, you can call the `compact` interface to free up disk space.
