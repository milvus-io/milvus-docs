...
For Arabic text, use the built-in [`arabic`](../analyzer/arabic-analyzer.md) analyzer in most cases. The built-in analyzer includes Arabic stemming together with Arabic normalization, decimal digit normalization, and Arabic stop-word removal. Use the `stemmer` filter directly only when you need to build a custom analyzer pipeline.
