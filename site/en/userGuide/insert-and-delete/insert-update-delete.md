...
- **Safe retries**: An `insert` whose response is lost cannot be retried safely, because the retry writes the rows a second time. Enabling idempotent insert on the collection, which is off by default, lets you send an idempotency key so that a retry is applied at most once and returns the original result. Sending a key before enabling it is rejected. For details, refer to [Idempotent Requests](idempotent-requests.md).
