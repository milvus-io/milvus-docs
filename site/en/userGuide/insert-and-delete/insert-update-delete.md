...
- **Safe retries**: An `insert` whose response is lost cannot be retried safely, because the retry writes the rows a second time. Send an idempotency key with the request so that a retry is applied at most once and returns the original result. For details, refer to [Idempotent Requests](idempotent-requests.md).
