...
By default the Milvus log is directly print on standard output (stdout) and standard error (stderr), we highly recommend you redirecting log to a persistent volume in production. To redirect log to a specified path, you can change the `log.file.rootPath` parameter in **milvus.yaml** file.
