...
**Note**: Standalone mode uses Woodpecker as the default message queue and enables the Streaming Node component. Standalone deployments run Woodpecker **embedded** in the Milvus pod; the dedicated Woodpecker **service** (separate pods) is used for **distributed/cluster** deployments only. For details, refer to the [Architecture Overview](architecture_overview.md) and [Use Woodpecker](woodpecker.md).
...
**Wait for all pods to show "Running" status.** With the v{{var.milvus_release_version}} configuration, you should see pods similar to:
