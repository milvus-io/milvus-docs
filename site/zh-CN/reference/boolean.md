...
表达式计算结果将输出布尔值——真（TRUE）或假（False）。在搜索向量时，Milvus 通过表达式进行标量过滤。查看 [Python SDK API](/api-reference/pymilvus/v{{var.milvus_python_sdk_version}}/api/collection.html) 参考以了解表达式使用说明。
