...
数据插入本身并不是一个 CPU 密集型操作，但是由于新插入的数据所在 segment 的大小可能还未达到自动创建索引的阈值，在查询时只能对其采用暴搜的方式，查询性能就会降低。
 ...
 参数 `rootcoord.minSegmentSizeToEnableIndex` 决定了 segment 自动构建索引的日志文件最小行数，默认值为 {{var.index_building_threshold}}。更多详情，请见[系统配置](configuration_cluster-advanced.md#系统行为配置)。
