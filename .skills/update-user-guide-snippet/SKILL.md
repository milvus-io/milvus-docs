...
description: Create, update, and fix the code snippets in the Milvus user guide pages under site/en/userGuide/ (on the currently checked-out version branch of milvus-docs) so they match each SDK's latest API, using the pymilvus snippets as the reference baseline. The version line is the current git branch (e.g. v3.0.x → site/en/userGuide/); naming a version (e.g. "更新2.6的user guide脚本" → v2.6.x) switches to that branch. Use when the user asks to update/refresh/fix/check the scripts or code snippets in the user guide, mentions "user guide" / userGuide, or asks to add missing SDK snippets (e.g. cpp).
---
...
- **Version selection is by Milvus major version, and in `milvus-docs` the
  version is the git branch you are on.** Each major line has its own branch:
  `v3.0.x` (Milvus 3.0), `v2.6.x` (Milvus 2.6), etc. There is **no** version
  prefix in the doc paths — snippets live under `site/en/userGuide/` on the
  current branch.
  - If the user does **not** specify a version → update only the **latest major
    version** line (currently `v3.0.x`). Work on the currently checked-out
    branch; if you are on an older line, checkout `v3.0.x` first. Never touch
    older lines by default.
  - If the user names a version (e.g. "更新2.6的user guide脚本" / "update the 2.6
    user guide") → **switch to that branch first** (`git checkout v2.6.x`), then
    update its `site/en/userGuide/`. If the working tree has uncommitted
    changes, stop and ask rather than stashing/overwriting.
  - The snippets live under `site/en/userGuide/**/*.md` on the working branch.
...
0. **Pre-flight: refresh the local milvus-docs checkout** (same rule as the
...
1. **Determine scope**: resolve the version line per the Scope rules (default
   latest major version, e.g. `v3.0.x` — checkout that branch if not already on
   it; or the explicitly named line, e.g. `v2.6.x` — checkout that branch). Only
   the working branch's `site/en/userGuide/` is touched.
2. **For each user guide page** (`site/en/userGuide/**/*.md` on the working
   branch):
...
`milvus-docs` publishes by pushing directly to the version branch (each branch
is one Milvus release line; the `ci.yml` GitHub Action converts the docs and
pushes them to web-content). So:

- without an explicit "commit and push to the remote" / "提交到远程仓库" (or
  "open a PR" / "提交 PR"), stop at local edits;
- when asked, create a branch named `docs/user-guide-snippets` (branched off the
  **target version branch**, e.g. `v3.0.x`), one **signed** commit
  (`git commit -sm`, title `Update user guide code snippets`), push, and open a
  PR against the target version branch if requested. Never force-push or delete
  a remote branch without approval.

> Note: pushing a PR branch never triggers the publish Action (that only fires
> on the version branch itself), so publishing to the live docs requires a
> separate merge/push to the version branch.
